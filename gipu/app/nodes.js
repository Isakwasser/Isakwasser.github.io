[
	"УЧЁНЫЙ СОВЕТ",
	"РЕКТОР",
	"ИФ и М",
	"Кафедра информатики и математки",
	"Кафедра физики и дидактики физики",
	"Кафедра металлургии",
	'Педагогический технопарк "Кванториум" им. В.Г. Разумовского',
	"ПРОРЕКТОР ПО ОБРАЗОВАТЕЛЬНОЙ ДЕЯТЕЛЬНОСТИ",
	"Учебное управление",
	"Центр развития компетенций",
	"Приемная комиссия",
	"ИЛФ",
	"Кафедра истории и социально-гуманитарных дисциплин",
	"Кафедра иностранных языков и удмуртской филологии",
	"П и ХО",
	"Кафедра дошкольного и начального образования",
	"Кафедра музыкального образования",
	"Кафедра физической культуры и МБД",
	"Филиал в г. Ижевске",
	"СК и Ф",
	"Кафедра русского языка и литературы",
	"Кафедра педагогики и психологии",
	"Центр информатизации и дистанционного обучения",
	"Отдел дополнительного образования",
	"Центр содействия трудоустройству выпускников",
	"Технопарк дошкольного и начального образования",
	"Технопарк универсальных педагогических компетенций",
	"Проректор по развитию имущественного комплекса",
	"Имущественный комплекс",
	"Главный специалист по ГО, ЧС и КБ",
	"ПРОРЕКТОР ПО НАУЧНОЙ И МЕЖДУНАРОДНОЙ ДЕЯТЕЛЬНОСТИ",
	"Отдел по научной деятельности",
	"Отдел международнего сотрудничества",
	"Аспирантура",
	"Научная библиотека",
	'Издательско-полиграфический центр "Экспресс"',
	"Помощник ректора",
	"Общий отдел",
	"Помощник ректора по стратегическому развитию",
	"Помощник ректора по правовым вопросам и безопасности",
	"Проректор по ВР и МП",
	"Управление по В и СР",
	"Центр досуга и творчества",
	"Отдел по работе с иностранными студентами и соц.вопросам",
	"Центр студенческих инициатив",
	"Служба социально-психологической поддержки студентов",
	"ДоброЦентр",
	"Спортклуб",
	"Санаторий-профилакторий",
	"Комбинат студенческого питания",
	"Совет ветеранов",
	"Совет обучающихся",
	"Центр истории ГИПУ",
	"УБУ и ФК",
	"Контрактная служба",
	"АКУ",
	"Гл. специалист по охране труда",
	"Юрисконсульт",
	"Профсоюз преподавателей и сотрудников",
	"Проектный офис",
	"Профком студентов и аспирантов",
].map((item, index) => ({ id: index + 1, label: item }));
